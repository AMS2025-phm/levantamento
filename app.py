...
def index():
    ...
    return render_template('index.html', data_hoje=data_hoje, tipos_piso=TIPOS_PISO, tipos_medida=TIPOS_MEDIDA, tipos_parede=TIPOS_PAREDE)
...
def exportar_excel_e_enviar_email():
    ...
    ws_detalhe.append(["Localidade", "Unidade", "Data", "Responsável", "Tipo de Piso", "Vidros Altos", "Vidros Altos - Risco", "Paredes", "Estacionamento", "Gramado", "Sala de Curativo", "Sala de Vacina", "Qtd Funcionários"])
    ws_detalhe.append([local, unidade, info.get("data", ""), info.get("responsavel", ""), ", ".join(info.get("piso", [])), info.get("vidros_altos", ""), "Sim" if info.get("vidros_altos_risco") else "Não", ", ".join(info.get("paredes", [])), "Sim" if info.get("estacionamento") else "Não", "Sim" if info.get("gramado") else "Não", "Sim" if info.get("curativo") else "Não", "Sim" if info.get("vacina") else "Não", info.get("qtd_func", "")])
    todas_as_medidas = info.get("medidas", [])
    categorias = {"Sanitários e Vestiários": [m for m in todas_as_medidas if m.get("tipo") == "Sanitário-Vestiário"], "Áreas Internas": [m for m in todas_as_medidas if m.get("tipo") == "Área Interna"], "Áreas Externas": [m for m in todas_as_medidas if m.get("tipo") == "Área Externa"], "Vidros": [m for m in todas_as_medidas if m.get("tipo") == "Vidro"]}
